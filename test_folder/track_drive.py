...
def ar_callback(data):
    ...
    for i in data.markers:
        ...
        ar_msg["DX"].append(i.pose.pose.position.x) # X값을 리스트에 추가
        ar_msg["DZ"].append(i.pose.pose.position.z) # Z값을 리스트에 추가
...
# ====================================
# 카메라 이미지에서 정답 이미지를 찾아 
# 좌우 위치관계에 따라 -1/0/+1 값을 반환하는 함수
# ====================================
def object_detect():
    ...
    target_found, other_found, both_found = False, False, False
    target_position, other_position = 0, 0
    
    # 정답 그림과 다른 그림, 모두 2장의 그림을 찾을 때까지 계속 인식을 시도한다.
    while (both_found == False):
        ...
        for i in range(len(scores)):
            if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
                ...
                # target 또는 other 그림을 발견하면 바운딩박스의 중앙지점의 X좌표값을 계산한다
                if object_name == 'stop':  # 정답 그림의 ID 값을 여기에 적는다 
                    target_found = True
                    target_position = (xmin+xmax)//2
                    print("target found:", target_position)
                                           
                if object_name == 'child':  # 정답이 아닌 그림의 ID 값을 여기에 적는다 
                    other_found = True
                    other_position = (xmin+xmax)//2
                    print("other found:", other_position)
     
                detected_object_num = detected_object_num + 1

        # 총 몇 장의 그림을 찾았는지 출력한다. 
        print("#################", detected_object_num, "objects")
		
        # 발견한 그림이 총 2장이고 하나가 target 그림, 다른 하나가 other 그림이면 both_found를 True로 한다.
        if (detected_object_num==2) and (target_found == True) and (other_found == True):
            both_found = True
    ...
    if (other_position < target_position):
        # 오른쪽에 target 그림이 있는 경우 1값을 리턴한다 
        return 1

    elif (target_position < other_position):
        # 화면 왼쪽에 target 그림이 있는 경우 -1값을 리턴한다
        return -1
...
def start():
    ...
    # 객체인식에 파일들이 어디에 있는지 경로 정보를 launch 파일에서 읽어들입니다. 
    PATH_TO_CKPT = rospy.get_param('~ckpt_path','/home/pi/xycar_ws/src/study/track_drive/src/detect.tflite')
    PATH_TO_LABELS = rospy.get_param('~label_path','/home/pi/xycar_ws/src/study/track_drive/src/labelmap.txt')
    ...
    while not rospy.is_shutdown():
        ...
        while drive_mode == TRAFFIC_SIGN:
            ...
            if (result == True):
                # 신호등이 파란불이면 미로주행 SENSOR_DRIVE 모드로 넘어갑니다.
        ...
        while drive_mode == SENSOR_DRIVE:
            ...
            if (result == True):
                ...
                stop_car(20) # 2초 정차 (이 시간을 바꾸는건 대회규정 위반임)
        ...
        # ======================================
        # 차선을 보고 주행합니다.
        # 횡단보도가 발견되면 잠시 정차했다가 다시 출발합니다.
        # AR태그가 발견되면 다른 주행모드로 넘어갑니다. 
		# 발견된 AR태그의 ID가 '1'이면 객체인식 모드로 (OBJECT_DETECT) 넘어갑니다. 
		# 발견된 AR태그의 ID가 '2'이면 주차 모드로 (PARKING) 넘어갑니다.
        # ======================================
        while drive_mode == LANE_DRIVE:
            ...
            if found:
                # 차선인식이 됐으면 차선의 위치정보로 조향각을 결정합니다. 
                new_angle = (((x_left + x_right) //2 ) - 320 ) # 적절한 조향각을 찾아야 합니다.

                if ( -30 < new_angle < 30 ):
                    new_speed = 12 # 적절한 속도를 찾아야 합니다.
                else: 
                    new_speed = 8 # 적절한 속도를 찾아야 합니다.

                drive(new_angle, new_speed)  
				
            else:
                # 차선인식이 안됐으면 기존 값을 사용하여 주행합니다. 	
                drive(new_angle, new_speed)

            # 전방에 AR태그가 보이는지 체크합니다.             
            ar_ID, distance, z_pos, x_pos = check_AR()

            if (distance < 1.0) and (ar_ID == 1): # Obejct Detection Sign
                # 1미터 안에 AR태그가 있고 ID값이 '1'이면 객체인식 OBJECT_DETECT 모드로 넘어갑니다.  
                print("AR_ID=",ar_ID," Z_pos=",z_pos," X_pos=",x_pos)
                drive_mode = OBJECT_DETECT  
                print ("----- Object Detection Start... -----")

            elif (distance < 0.5) and (ar_ID == 2): # Parking Sign
                # 1미터 안에 AR태그가 있고 ID값이 '1'이면 주차 PARKING 모드로 넘어갑니다.  
                print("AR_ID=",ar_ID," Z_pos=",z_pos," X_pos=",x_pos)
                drive_mode = PARKING  
                print ("----- Parking Start... -----")
        
        # ======================================
        # 앞에 놓여져 있는 그림을 보고 정답 그림을 찾습니다.
        # 차량을 정답 그림이 있는 방향으로 주행시킵니다.  
        # ======================================
        while drive_mode == OBJECT_DETECT:
            ...
            if (object_position == -1):
                # 정답 그림이 왼쪽에 있는 것으로 판단되면 왼쪽으로 돌아나갑니다. 
                print("... Target image is on Left")         
                # 차선인식 모드로 바꾸어 주행을 계속합니다.
                drive_mode = LANE_DRIVE
				
            elif (object_position == +1):
                # 정답 그림이 오른쪽에 있는 것으로 판단되면 오른쪽으로 돌아나갑니다. 
                print("... Target image is on Right")
                # 차선인식 모드로 바꾸어 주행을 계속합니다.
                drive_mode = LANE_DRIVE
        ...
        while drive_mode == PARKING:
            ...
            if (ar_ID == 99):
                ...
                if (z_pos > 0.2):
                    ...
                    # Z값이 20센치 이하이면 차량을 세우고 종료 FINISH 모드로 넘어갑니다.     
                    new_angle = 0
                    new_speed = 0                      
                    drive_mode = FINISH  
