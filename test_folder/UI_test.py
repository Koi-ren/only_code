...
#-------------제한시간----------------------------------------제한시간-----------------------------------------
def timeout():
    """타임아웃 함수: 10분 후에 프로그램 종료"""
    time.sleep(10)
    print("프로그램이 10분을 초과하여 종료됩니다.")
    sys.exit()
...
#                    print(f"경로에서 벗어났거나 GPS 오차범위에 들어가지 않았습니다.")
            ...
            next_planning_point_number = planning_point_number + 300
            ...
            first_angle_to_move = angle_to_move = calculate_angle(heading_degree, first_target_bearing)
...
#                print(f"가장 근사한 좌표는:\n포인트 넘버: {planning_point_number} \n 다음 포인트까지남은 거리: {remain_distance*1000}m, 다음 포인트로의 각도{real_steering_angle}")
    ...
    ''' % ("True" if planning_point_data_bool else "False", 
            planning_point_number,
