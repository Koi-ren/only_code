...
import cv2
import numpy as np
from plt_motor import plt_motor  # plt_motor.py 파일에서 함수 임포트
...
def receive_steering_angle(pipe):
    global speed, stop_received  # speed 값과 stop_received 플래그를 제어하기 위해 global로 설정
    while running:
        try:
...
if __name__ == "__main__":
    ...
    p = Process(target=plt_motor, args=(child_pipe,))
    p.start()
