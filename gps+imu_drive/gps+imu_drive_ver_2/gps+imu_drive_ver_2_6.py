...
if __name__ == "__main__":
    ...
    #            state.update(lat = imu_lat, lon = imu_lon, heading_degree = heading_degree)  # 제어 입력에 따라 차량 상태 업데이트
            ...
            """
            현재 GPS 좌표(imu_lat, imu_lon)에 가장 근접한 경로 좌표를 검색하고, 포인트 정보 및 방위각을 계산합니다.
            """
            # 가장 가까운 경로 좌표를 찾습니다.
            ...
            # 위도와 경도 인덱스가 다를 때 각각 처리
                ...
                if is_within_tolerance(imu_lat, imu_lon, lat_closest_value, lon_data[lat_index], tolerance):
                    planning_point_number = planning_point_data[lat_index]
                    previous_planning_point_number = planning_point_number
                    planning_point_data_bool = True
                ...
                if is_within_tolerance(imu_lat, imu_lon, lat_data[lon_index], lon_closest_value, tolerance):
            ...
            # 다음 포인트를 향한 각도 계산
            if planning_point_number + 1 < len(planning_point_data):
                remain_distance = calculate_distance(imu_lat, imu_lon, lat_data[planning_point_number+1], lon_data[planning_point_number])
                target_degree = calculate_bearing(imu_lat, imu_lon, lat_data[planning_point_number], lon_data[planning_point_number])
                diff_angle_value = calculate_angle(bearing, target_degree)
                new_angle = diff_angle_value
            else:
                new_angle = 0  # 마지막 포인트에서는 이동 각도 없음

            # 상태 출력
            if not planning_point_data_bool:
                print("현재 가장 가까운 스타팅 포인트로 이동합니다.")
                print(f"남은 거리: {remain_distance*1000}m")
            else:
                print(f"다음 포인트: {planning_point_number + 10}")
                print(f"남은 거리: {remain_distance*1000}m")
