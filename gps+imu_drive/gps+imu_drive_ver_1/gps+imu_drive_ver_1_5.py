...
gps_save_path = "C:/Users/plane/Desktop/park_ws/gps+imu_drive/gps_coordinates.xlsx"
gps_workbook = openpyxl.Workbook()
gps_sheet = gps_workbook.active
gps_sheet.title = "gps_coordinates"
gps_sheet.append(["Point Number", "Latitude", "Longitude"])
            ...
            if ser.in_waiting > 0:
                ...
                if line.startswith("Lat:"):
                    ...
                    if len(parts) >= 6:
                        ...
                        if not (36.9 <= lat <= 39.14 and 126.5 <= lon <= 128.5):
                            lat, lon = None, None
...
def main_loop():
    ...
    while True:
        ...
        if lat is not None and lon is not None:
            ...
            save_to_excel(gps_sheet, i, lat, lon)
        else:
            print(f"GPS 신호 수신 못함")

        if prev_lat == lat or prev_lon == lon:
            imu_lat, imu_lon = update_position(lat, lon, imu_euclidean_distance, heading_degrees)
            save_to_excel(gps_imu_sheet, i, imu_lat, imu_lon)

        if prev_lat != lat or prev_lon != lon:
            imu_lat, imu_lon = None, None
        ...
        if i != 0 and i % 2100 == 0:
            try:
                gps_workbook.save(gps_save_path)
                print('데이터 저장 중')
            except PermissionError:
                print("파일 저장 권한 오류 발생. 엑셀 파일이 열려있지 않은지 확인하세요.")
            except Exception as e:
