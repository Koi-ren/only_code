...
dt, imu_velocity, imu_distance, prev_accel = 0.00333, {0:0, 1:0, 2:0}, {0:0, 1:0, 2:0}, [0, 0, 0] # 100Hz 샘플링

imu_distance_save_path = "C:/Users/plane/Desktop/park_ws/gps+imu_drive/imu_distance.xlsx"
imu_distance_workbook = openpyxl.Workbook()
imu_distance_sheet = imu_distance_workbook.active
imu_distance_sheet.title = "imu_distance"
imu_distance_sheet.append(["x Axis", "y Axis", "z Axis"])

def save_to_excel(sheet, x_Axis, y_Axis, z_Axis):
    """좌표 데이터를 엑셀에 저장하는 함수"""
    sheet.append([f"{x_Axis}", f"{y_Axis}", f"{z_Axis}"])
...
def update_velocity_and_distance_trapezoid():
    ...
    imu_velocity[0] += ((acceleration[0]*-9.8 + prev_accel[0]*-9.8) / 2) * dt
    imu_velocity[1] += ((acceleration[1]*-9.8 + prev_accel[1]*-9.8) / 2) * dt
...
def main_loop():
    ...
    while True:
        ...
        print("\n")
        save_to_excel(imu_distance_sheet, imu_distance[0], imu_distance[1], imu_distance[2])

        if i != 0 and i % 1500 == 0:
            try:
                imu_distance_workbook.save(imu_distance_save_path)
                print('데이터 저장 중')
        ...
        if q == 300:
            ...
            imu_velocity[0], imu_velocity[1], imu_velocity[2] = 0, 0, 0
            q = 0
        q += 1
        print(q)
