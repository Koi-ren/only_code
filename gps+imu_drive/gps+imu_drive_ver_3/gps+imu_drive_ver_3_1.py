...
# gps+imu 융합 좌표 저장
gps_imu_save_path = "C:/park_ws/gps+imu_drive/gps+imu_coordinates_hans_pc.csv"

#경로 계획 좌표 읽기
planning_csv_path = "C:/park_ws/gps+imu_drive/bunsudae_planning.csv"
...
i = planning_point_number = point_number= 0
prev_lat, prev_lon = None, None
target_bearing = remain_distance = angle_to_move = real_steering_angle = 0
previous_angle_to_move = previous_remain_distance = previous_target_bearing = 0
first_angle_to_move = first_remain_distance = first_target_bearing = 0
...
#    declination_angle = (-8.0 + (-29.0 / 60.0)) / (180 / math.pi) #경기장 기준
    declination_angle = (-9.0 + (-2.0 / 60.0)) / (180 / math.pi) #학교 기준
...
#                    print(f"경로에서 벗어났거나 GPS 오차범위에 들어가지 않았습니다.")
            ...
            next_planning_point_number = planning_point_number + 500
            previous_planning_point_number = planning_point_number

            if planning_point_number >= len(lat_data) - 500 or planning_point_number == len(lon_data) - 500:
            ...
            if planning_point_data_bool == False:
                if previous_planning_point_number == 0:
                    ...
                    print(f"경로에서 벗어나거나 GPS 오차범위에 들어가지 않았습니다. 이전 위치 기반으로 다음 위치로 향합니다.")
                    previous_target_bearing = calculate_bearing(imu_lat, imu_lon, lat_data[previous_planning_point_number + 3], lon_data[previous_planning_point_number +3])
                    previous_remain_distance = calculate_distance(imu_lat, imu_lon, lat_data[previous_planning_point_number + 3], lon_data[previous_planning_point_number +3])
                    previous_angle_to_move = calculate_angle(heading_degree, previous_target_bearing)
                    if abs(previous_angle_to_move) >= 43.0:
                        if  previous_angle_to_move < 0.0:
                            real_steering_angle = -43.0
                        else :
                            real_steering_angle = 43.0
                    else:
                        real_steering_angle =  first_angle_to_move
                    print(f"가장 근사한 좌표는:\n포인트 넘버: {previous_planning_point_number} \n 다음 포인트까지남은 거리: {previous_remain_distance*1000}m, 다음 포인트로의 방위각{first_target_bearing}")
                        

            if planning_point_data_bool == True:
                print("현재 경로에 위치합니다") 
                target_bearing = calculate_bearing(imu_lat, imu_lon, lat_data[next_planning_point_number], lon_data[next_planning_point_number])
                remain_distance = calculate_distance(imu_lat, imu_lon, lat_data[next_planning_point_number], lon_data[next_planning_point_number])
