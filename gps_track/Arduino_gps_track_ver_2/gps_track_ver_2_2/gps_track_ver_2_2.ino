...
#define pds 8
int switchState = 0;
long lastTime = 0; // I2C 트래픽을 u-blox 모듈로 제한

void pushbutton(int currentState)
{
  if (currentState != switchState)
  {
    switchState = currentState;
    Serial.print(switchState);  // 변경된 버튼 상태 출력
  }
}
...
{
  Serial.begin(9600);
  pinMode(pds, INPUT); // 필요에 따라 INPUT_PULLUP으로 변경 가능
...
void loop()
{
  ...
  if (millis() - lastTime > 1000)
  {
    ...
    int currentState = digitalRead(pds);
    Serial.print(F(" Button_Signal: "));
    pushbutton(currentState);
