...
def initialize_particles(lat, lon, num_particles):
    ...
    for _ in range(num_particles):
        particle = [lat + random.uniform(-0.001, 0.001), lon + random.uniform(-0.001, 0.001)]
...
def update_weights(particles, lat, lon):
    ...
    for particle in particles:
        distance = np.sqrt((lat - particle[0]) ** 2 + (lon - particle[1]) ** 2)
        weight = np.exp(-distance ** 2 / 0.001**2)  # Gaussian 분포를 사용한 가중치 계산
...
gps_counter = 0  # GPS 데이터 수신 횟수를 세는 카운터
