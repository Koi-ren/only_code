...
def read_gps_data(ser):
    ...
    try:
        ...
        if "GNSS_1:" in line:
            ...
            lat1 = float(lat_line.split("Lat: ")[1].split(" Long: ")[0]) / 10000000.0
            lon1 = float(lat_line.split("Long: ")[1].split(" Alt: ")[0]) / 10000000.0
            alt1 = float(lat_line.split("Alt: ")[1]) / 10000000.0

        line = ser.readline().decode('utf-8').strip()
        if "GNSS_2:" in line:
            # GNSS_2 데이터 처리
            lat_line = ser.readline().decode('utf-8').strip()
            lat2 = float(lat_line.split("Lat: ")[1].split(" Long: ")[0]) / 10000000.0
            lon2 = float(lat_line.split("Long: ")[1].split(" Alt: ")[0]) / 10000000.0
            alt2 = float(lat_line.split("Alt: ")[1]) / 10000000.0
...
def particle_filter(particles, weights, measurements):
    ...
    for i in range(NUM_PARTICLES):
        ...
        particles[i, 0] += random.gauss(0, 0.0000001)  # 이동 범위 조정
        particles[i, 1] += random.gauss(0, 0.0000001)
