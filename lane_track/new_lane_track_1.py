...
#    vx, vy, x, y = output[0], output[1], output[2], output[3]
    output = cv2.fitLine(lines, cv2.DIST_L2, 0, 0.01, 0.01)
    vx, vy, x, y = output[0].item(), output[1].item(), output[2].item(), output[3].item()

#    x1, y1 = int(((img.shape[0][0]-1)-y)/vy*vx + x), img.shape[0][0]-1
#    x2, y2 = int(((img.shape[0][0]/2+100)-y)/vy*vx + x), int(img.shape[0][0]/2+100)
    x1, y1 = int(((img.shape[0]-1)-y)/vy*vx + x) , img.shape[0]-1
    x2, y2 = int(((img.shape[0]/2+100)-y)/vy*vx + x) , int(img.shape[0]/2+100)
#    x1, y1 = int(((width-1)-y)/vy*vx + x) , width-1
#    x2, y2 = int(((width/2+100)-y)/vy*vx + x) , int(width/2+100)
#    x1, y1 = int(((img.shape[0]-1)-vy)/vy*vx + vx) , img.shape[0]-1
#    x2, y2 = int(((img.shape[0]/2+100)-vy)/vy*vx + vx) , int(img.shape[0]/2+100)

    result = [x1,y1,x2,y2]
    return result

#image = cv2.imread('C:/new/slope_test.jpg') # 이미지 읽기
#cap = cv2.VideoCapture('C:/Users/plane/Advanced-lane-finding/challenge_video.mp4')
cap = cv2.VideoCapture('C:/new/solidWhiteRight.mp4')
...
while(cap.isOpened()):
    ...
    draw_fit_line(line_draw_img, left_fit_line)
    draw_fit_line(line_draw_img, right_fit_line)
