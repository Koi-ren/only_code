...
#=============================================
CAM_FPS = 20  # 카메라 FPS 초당 30장의 사진을 보냄
WIDTH, HEIGHT = 640, 480  # 카메라 이미지 가로x세로 크기
...
def usbcam_callback(data):
    global image
    if data is not None:  # 받아온 데이터가 비어있지 않은 경우에만 이미지 변수에 할당
...
def lane_detect():
    global image
    if image is None or image.size == 0:  # 이미지가 비어있는 경우
        return False, 0, 0
    ...
    if roi_img is None or roi_img.size == 0:  # roi_img가 비어있는 경우
        return False, 0, 0
