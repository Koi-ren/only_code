...
bool sendData1 = false; // Signal transmitted and received via ESP NOW(Online)
bool sendData2 = false; // Signal transmitted and received via Serial
bool sendData3 = false; // Signal transmitted and recived via ESP NOW(Offline)
...
void loop() {
  ...
  if (button == 1){
    sendData3 = true;
    esp_now_send(dest.peer_addr, (uint8_t *)&sendData3, sizeof(sendData3));
  }
  else if (button == 0){
    sendData3 = false;
    esp_now_send(dest.peer_addr, (uint8_t *)&sendData3, sizeof(sendData3));
