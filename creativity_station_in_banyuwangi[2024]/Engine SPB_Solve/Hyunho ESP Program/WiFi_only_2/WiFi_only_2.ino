      ...
      if (client.available()) {             // If there is data available to read from the client
        ...
        if (c == '\n') {                    // If the character is a newline
          ...
          if (currentLine.length() == 0) {
            ...
            client.println("<title>IoT 기반 자동 압력 제어</title>");
            client.println("<style>");
            client.println("body { font-family: Arial, sans-serif; text-align: center; }");
            client.println(".background-text { font-size: 24px; margin: 20px; }");
            client.println(".form-box { display: inline-block; margin-top: 50px; }");
            ...
            client.println(".image-container img { width: 300px; margin-top: 20px; }");
            client.println("</style>");
            client.println("</head>");
            client.println("<body>");
            client.println("<div class=\"background-text\">IoT 기반 바이오가스 시스템의 자동 압력 제어</div>");
            client.println("<div class=\"form-box\">");

            // 버튼 상태를 확인하고 적절한 버튼을 렌더링합니다.
            if (butState) {
            ...
            client.println("<img src=\"project-image.jpg\" alt=\"프로젝트 이미지\">");
            client.println("</div>");
            client.println("</body>");
            client.println("</html>");
            ...
            client.println();
            requestHandled = true;  // 요청이 처리되었음을 표시합니다.
            break;
        } 
