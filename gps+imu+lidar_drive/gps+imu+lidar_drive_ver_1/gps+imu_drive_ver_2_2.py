            ...
            if ser.in_waiting > 0:
                ...
                if line.startswith("Lat:"):
                    ...
                    if len(parts) >= 6:
                        ...
                        #if not (35.0 <= lat <= 39.0 and 125.0 <= lon <= 128.0):
                        if not (35.0 <= lat <= 39.0 and 127.0 <= lon <= 130.0):
                            lat, lon = None, None
...
def heading_degree_found():
    ...
    heading = math.atan2(magnetometer[1], magnetometer[0]) + (-9.0 + (-2.0 / 60.0)) / (180 / math.pi)
...
def imu_part():
    find_ttyUSB()
    try:
        hf_imu = serial.Serial(port="COM4", baudrate=921600, timeout=0.5)
